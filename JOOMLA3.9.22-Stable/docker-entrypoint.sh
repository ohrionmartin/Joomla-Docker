...
if [ -d "$installFolder" ]; then
  ...
  mysql -u "$DBUSER" -p"$DBPASS" -h "$DBHOST" -D "$DBNAME" -e "INSERT INTO ${DBPREFIX}_users (id, name, username, email, password, registerDate, lastvisitDate, lastResetTime, params, block, requireReset) VALUES(${USERID}, '${WEBSITESUNAME}', '${WEBSITESUSERNAME}', '${WEBSITESEMAIL}', '${PASSWORDHASH}', '${TODAY}', '2020-06-08 00:00:00', '2020-01-01 00:00:00', '', 0, '${WEBSITESPASSRESET}')"
  ...
  # whith this we can install multi extensions
  if [ -f "/home/docker/scripts/packages.vdm" ]; then
    while read ZIP_URL; do
      # random fodler for each install
      folderName=$(getRandomString)
      # get the zip package
      curl -o tmp/package.zip -SL "$ZIP_URL"
      # make the folder for the zip package
      mkdir -p tmp/$folderName
      # unzip the package
      unzip -d tmp/$folderName tmp/package.zip
      # install the package
      php /var/www/html/cli/vdmInstallExtension.php -p tmp/$folderName
